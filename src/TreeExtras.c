...
TypeDistributionItem **rankingNumberDistribution(int n) {
    int i;
    TypeDistributionItem **emp;
    emp = (TypeDistributionItem**) malloc(n*sizeof(TypeDistributionItem*));
    emp--;
    emp[1] = (TypeDistributionItem*) malloc(2*sizeof(TypeDistributionItem));
    emp[1][0].val = 0.;
    emp[1][0].eff = 1.;
    emp[1][1].val = sqrt(-1);
    for(i=2; i<=n; i++) {
        int j, ind = 0, size = INC_BUFFER_DISTRIB;
        emp[i] = (TypeDistributionItem*) malloc(size*sizeof(TypeDistributionItem));
        for(j=1; j<i; j++) {
            int k, l;
            for(k=0; !isnan(emp[j][k].val); k++)
                for(l=0; !isnan(emp[i-j][l].val); l++) {
                    if(ind >= size) {
                        ...
                        emp[i] = (TypeDistributionItem*) realloc((void*) emp[i], size*sizeof(TypeDistributionItem));
                    }
...
printf("i %d tot eff %.2lE/%.2lE %.2lE/%.2lE\n", i, suma, numberTrees(i), sumb, exp(2*gsl_sf_lnfact(i-1)+log(i)));
        size = ind;
        qsort(emp[i], size, sizeof(TypeDistributionItem),compareEmpiricalItem);
       ind=0;
        for(j=1; j<size; j++) {
       ...
       emp[i] = (TypeDistributionItem*) realloc((void*) emp[i], (ind+1)*sizeof(TypeDistributionItem));
    }
    return emp+1;
}
